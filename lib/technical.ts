...
export function analyze(candles: Candle[], interval: string = '1h'): TechnicalResult {
  ...
  // NEUTRAL:
  //   EMA20 > EMA50 && RSI > 50 → Kuchli BUY
  //   EMA20 < EMA50 && RSI > 50 → Ehtiyotkor BUY
  //   EMA20 < EMA50 && RSI < 50 → Kuchli SELL
  ...
  if (trend === 'BEARISH') {
    ...
    // EMA stack bearish yoki teng, lekin RSI bullish
    side = 'BUY'
    neutralTone = 'caution'
  } else if (r < 50) {
    // EMA stack bullish yoki teng, lekin RSI bearish
    side = 'SELL'
    neutralTone = 'caution'
  } else {
    // RSI == 50 — stack bo'yicha, ehtiyotkor
