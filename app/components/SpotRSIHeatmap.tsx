  ...
  return (
    <section className="gwHeatmap">
      <style>{`
        ...
        .gwPrice { font-size:.72rem; margin-bottom:5px; font-weight:750; opacity:.96; }
        .gwSignals { display:flex; justify-content:center; gap:8px; margin-top:4px; font-size:.72rem; font-weight:800; }
        .gwSignal { display:inline-flex; align-items:center; gap:2px; }
        .gwSignalLabel { font-size:.58rem; font-weight:600; opacity:.65; }
        .gwState { display:block; font-size:.63rem; margin-top:3px; opacity:.78; }
      ...
      <div className="gwHeatHeader">
        ...
        <div className="gwHeatMeta">
          ...
          <span className="gwBadge">↑ ko'tarilish · ↓ tushish</span>
        ...
        <>
                  ...
                  {TIMEFRAMES.map((tf) => {
                    ...
                    return (
                      <button
                        ...
                        title={`${coin} ${tf} • Narx: ${formatPrice(cell?.price ?? null)} ${arrow(cell?.priceDirection ?? null)} • RSI: ${cell?.rsi == null ? 'N/A' : cell.rsi.toFixed(1)} ${arrow(cell?.rsiDirection ?? null)}`}
                      >
                        <div>
                          <div className="gwPrice">{formatPrice(cell?.price ?? null)}</div>
                          <div className="gwRsi">RSI {cell?.rsi == null ? '—' : cell.rsi.toFixed(1)}</div>
                          <div className="gwSignals">
                            <span className="gwSignal"><span className="gwSignalLabel">P</span>{arrow(cell?.priceDirection ?? null)}</span>
                            <span className="gwSignal"><span className="gwSignalLabel">R</span>{arrow(cell?.rsiDirection ?? null)}</span>
                          </div>
          ...
          {selected && (
            <div className="gwDetail">
              <strong>{selected.coin} / {selected.tf}</strong> — RSI(14): {payload?.data?.[selected.coin]?.[selected.tf]?.rsi ?? 'N/A'} {arrow(payload?.data?.[selected.coin]?.[selected.tf]?.rsiDirection ?? null)} • Narx: {formatPrice(payload?.data?.[selected.coin]?.[selected.tf]?.price ?? null)} {arrow(payload?.data?.[selected.coin]?.[selected.tf]?.priceDirection ?? null)} • P = price, R = RSI
            </div>
          )}
        </>
      )}
