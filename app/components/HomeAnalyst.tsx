    ...
    <div className="homeChartWrap">
      ...
      <div className="homeChartScroller">
        <svg viewBox={`0 0 ${W} ${H}`} className="homeChart" style={{width:`${zoom*100}%`,maxWidth:'none'}} role="img" aria-label={`${coin} ${interval} professional technical analysis`}>
          ...
          <text x={L} y="70" fill="#ff9f0a" fontSize="12" fontWeight="700">EMA 10 (toq sariq): {money(result.ema10)}</text>
          <text x={L+260} y="70" fill="#00c7e6" fontSize="12" fontWeight="700">EMA 20 (kok): {money(result.ema20)}</text>
...
export default function HomeAnalyst(){
  ...
  const [coin,setCoin]=useState(initial),[interval,setInterval]=useState('1h'),[data,setData]=useState(null as any),[loading,setLoading]=useState(true),[error,setError]=useState('')
  ...
  return <section className="homeAnalyst">
    ...
    {loading?<div className="homeLoading">Grafik va tahlil yuklanmoqda...</div>:error?<div className="homeLoading error">{error}</div>:r&&<>
      ...
      <p className="homeDisclaimer">Eslatma: Ushbu tahlil faqat axborot maqsadida. Investitsiya tavsiyasi emas. Savdo qilishdan oldin ozingiz tahlil qiling.</p>
    </>}
  </section>
}
