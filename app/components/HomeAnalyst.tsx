...
function CleanChart({candles,result,coin,interval}:{candles:Candle[];result:Result;coin:string;interval:string}){
  ...
  // Wide right-side gap so the straight arrow sits comfortably between last candle and price labels.
  const W=1500,H=780,L=62,R=420,T=48,MB=520,RT=565,RB=700
  ...
  // Arrow ends inside the gap, before the labels
  const xFuture=Math.min(W-R-30,lx+160)
  const zoneLeft=x(Math.max(0,candles.length-18))
  const zoneWidth=Math.max(80,lx+40-zoneLeft)
  const label=(yy:number,text:string,fill:string)=> <g><line x1={L} x2={W-R} y1={yy} y2={yy} stroke={fill} strokeWidth="1.6" strokeDasharray="9 7"/><rect x={W-R+16} y={yy-15} width="170" height="30" rx="5" fill={fill}/><text x={W-R+101} y={yy+5} textAnchor="middle" fill="#fff" fontSize="14" fontWeight="800">{text}</text></g>
  return <div className="homeChartWrap"><div className="chartZoomControls"><button onClick={()=>setZoom(z=>Math.min(1.8,+(z+0.2).toFixed(1)))}>＋</button><span>{Math.round(zoom*100)}%</span><button onClick={()=>setZoom(z=>Math.max(1,+(z-0.2).toFixed(1)))}>−</button><button onClick={()=>setZoom(1)}>Reset</button></div><div className="homeChartScroller"><svg viewBox={`0 0 ${W} ${H}`} className="homeChart" style={{width:`${zoom*100}%`,maxWidth:'none'}} role="img" aria-label={`${coin} ${interval} professional technical analysis`}>
    ...
    <line x1={lx} x2={W-R} y1={y(latest)} y2={y(latest)} stroke="#65d9ff" strokeDasharray="3 5"/><rect x={W-R+16} y={y(latest)-15} width="170" height="30" rx="5" fill="#1d9f59"/><text x={W-R+101} y={y(latest)+5} textAnchor="middle" fill="#fff" fontSize="14" fontWeight="800">{money(latest)}</text>
    <rect x={zoneLeft} y={y(result.entryHigh)} width={zoneWidth} height={Math.max(10,y(result.entryLow)-y(result.entryHigh))} fill="#1dbf6b" fillOpacity=".14" stroke="#20d67a" strokeOpacity=".5" rx="4"/>
    ...
    <line x1={lx+10} y1={y(latest)-2} x2={xFuture} y2={y(result.tp[0]||latest)} stroke="#20d67a" strokeWidth="2.2" strokeDasharray="8 6" markerEnd="url(#hcBull)"/>
