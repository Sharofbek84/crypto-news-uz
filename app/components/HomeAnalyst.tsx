...
function CleanChart({candles,result,coin,interval}:{candles:Candle[];result:Result;coin:string;interval:string}){
  ...
  const W=1600,H=780,L=70,R=230,T=48,MB=520,RT=565,RB=700
  const plotRight=W-R-65
  ...
  const latest=candles[candles.length-1]?.close||0, lx=x(candles.length-1)
  const fw=Math.min(plotRight,lx+120)
  const cw=Math.max(3,Math.min(11,(plotRight-L)/candles.length*.7))
  const label=(yy:number,text:string,fill:string)=><g><line x1={plotRight} x2={plotRight+28} y1={yy} y2={yy} stroke={fill} strokeWidth="1.6" strokeDasharray="7 6"/><rect x={plotRight+34} y={yy-15} width="145" height="30" rx="5" fill={fill}/><text x={plotRight+106} y={yy+5} textAnchor="middle" fill="#fff" fontSize="14" fontWeight="800">{text}</text></g>
  return <div className="homeChartWrap"><div className="chartZoomControls"><button onClick={()=>setZoom(z=>Math.min(1.8,+(z+0.2).toFixed(1)))}>＋</button><span>{Math.round(zoom*100)}%</span><button onClick={()=>setZoom(z=>Math.max(1,+(z-0.2).toFixed(1)))}>−</button><button onClick={()=>setZoom(1)}>Reset</button></div><div className="homeChartScroller"><svg viewBox={`0 0 ${W} ${H}`} className="homeChart" style={{width:`${zoom*100}%`,maxWidth:'none'}} role="img" aria-label={`${coin} ${interval} professional technical analysis`}>
    ...
    {[0,.2,.4,.6,.8,1].map(v=><line key={v} x1={L} x2={plotRight} y1={T+v*(MB-T)} y2={T+v*(MB-T)} stroke="#1c2a38"/>)}
    {[30,50,70].map(v=><line key={v} x1={L} x2={plotRight} y1={ry(v)} y2={ry(v)} stroke="#596474" strokeDasharray="5 7"/>)}
    ...
    <line x1={lx} x2={plotRight} y1={y(latest)} y2={y(latest)} stroke="#65d9ff" strokeDasharray="3 5"/><rect x={plotRight+34} y={y(latest)-15} width="145" height="30" rx="5" fill="#1d9f59"/><text x={plotRight+106} y={y(latest)+5} textAnchor="middle" fill="#fff" fontSize="14" fontWeight="800">{money(latest)}</text>
    ...
    <path d={`M${lx},${y(latest)-5} C${lx+35},${y(latest)-35} ${fw-30},${y(result.tp[0]||latest)+30} ${fw},${y(result.tp[0]||latest)}`} fill="none" stroke="#20d67a" strokeWidth="2" strokeDasharray="8 6" markerEnd="url(#hcBull)"/>
    ...
    <text x={plotRight-8} y={ry(70)-6} textAnchor="end" fill="#8c98a6" fontSize="12">70</text><text x={plotRight-8} y={ry(50)-6} textAnchor="end" fill="#8c98a6" fontSize="12">50</text><text x={plotRight-8} y={ry(30)-6} textAnchor="end" fill="#8c98a6" fontSize="12">30</text>
  </svg></div></div>
}
...
export default function HomeAnalyst(){
  ...
  const r=data?.result
  const risk=((r?.tp?.[0]||0)-(r?.entryHigh||0)),downside=((r?.entryLow||0)-(r?.invalidation||0)),rr=downside>0?risk/downside:0
